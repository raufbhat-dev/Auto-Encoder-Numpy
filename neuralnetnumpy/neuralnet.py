...
class NeuralNet:
    def __init__(self, loss_func, optimiser_method, learning_rate, epoch, partition_size, mode, network_arch,**kwargs):
        ...
        if optimiser_method == 'momentum':
            momentum = True
            self.optimiser = optimiser.GradientDescent(momentum, learning_rate,beta = kwargs['beta'])
        else:
            momentum = False
    ...
    def train(self, inputs, outputs):
        ...
        for i in range(self.epoch_count):
            for index, (inp_batch, out_batch) in enumerate(zip(inputs, Y)):  
                ...
                print('Epoch:{}  batch: {}/{}    Loss: {}'.format(i+1, index+1, self.partition_size, self.loss.loss), end='\r', flush=True)
            #print("",end="\n")
